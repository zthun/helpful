...
describe("ZEmailContactAddressBuilder", () => {
  ...
  beforeEach(() => {
    ...
    wolverine = new ZEmailContactBuilder()
      .address("wolverine@marvel.com")
      .display("Logan")
      .type("x-men")
      .build();
  ...
  it("should returns a comma separated list of items.", () => {
    ...
    expect(
      createTestTarget().addresses([gambit, wolverine]).address(x).build(),
    ).toEqual(expected);
  });
  ...
  it("should filter out falsy items.", () => {
    ...
    expect(
      createTestTarget()
        .addresses([gambit, wolverine, psylocke, x])
        .address("")
        .build(),
    ).toEqual(expected);
  });
