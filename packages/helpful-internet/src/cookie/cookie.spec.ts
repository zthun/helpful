...
describe("ZCookieBuilder", () => {
  ...
  describe("Properties", () => {
    ...
    it("should set the value.", () => {
      ...
      expect(createTestTarget().value(expected).build().value).toEqual(
        expected,
      );
    });
    ...
    it("should set the domain.", () => {
      ...
      expect(createTestTarget().domain(expected).build().domain).toEqual(
        expected,
      );
    });
    ...
    describe("Expiration", () => {
      it("should set the expiration date.", () => {
        ...
        expect(createTestTarget().expires(expected).build().expires).toEqual(
          expected,
        );
      });
      ...
      it("should set the expiration date to one day from this moment.", () => {
        const expected = new Date(
          Date.now() + ZCookieBuilder.MillisecondsOneDay,
        ).toJSON();
        expect(
          createTestTarget().expiresTomorrow().build().expires! >= expected,
        ).toBeTruthy();
      });

      it("should remove the expiration.", () => {
        expect(
          createTestTarget().expiresTomorrow().immortal().build().expires,
        ).toBeUndefined();
      });
    });
    ...
    describe("Same Site", () => {
      ...
      it("should be none.", () => {
        expect(createTestTarget().allowCrossSite().build().sameSite).toEqual(
          "none",
        );
      });
  ...
  describe("Authentication", () => {
    it("sets the name.", () => {
      ...
      expect(createTestTarget().authentication().build().name).toEqual(
        expected,
      );
    });
    ...
    it("sets the value.", () => {
      const expected = createGuid();
      expect(createTestTarget().authentication(expected).build().value).toEqual(
        expected,
      );
    });
    ...
    it("expires in 24 hours.", () => {
      ...
      expect(
        createTestTarget().authentication().build().expires! >= expected,
      ).toBeTruthy();
    });
  });
});
